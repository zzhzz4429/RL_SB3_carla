...
class LidarCenterNet(nn.Module):
    ...
    def forward_ego(self, rgb, lidar_bev, target_point, target_point_image, ego_vel, bev_points=None, cam_points=None, save_path=None, SAVE_PATH1=None,expert_waypoints=None,
        ...
        if debug and self.i % 2 == 0 and not (save_path is None):
            ...
            # save_path2 = os.path.join(SAVE_PATH1, f'fused_features_step{self.i}.pt')
            # torch.save(fused_features,save_path2)

            self.visualize_model_io(save_path, SAVE_PATH1,self.i, self.config, rgb, lidar_bev, target_point,
                            pred_wp, pred_bev, pred_semantic, pred_depth, bboxes, self.device,
                            gt_bboxes=None, expert_waypoints=expert_waypoints, stuck_detector=stuck_detector, forced_move=forced_move,fused_features=fused_features)
    ...
    def visualize_model_io(self, save_path,save_path1, step, config, rgb, lidar_bev, target_point,
                        pred_wp, pred_bev, pred_semantic, pred_depth, bboxes, device,
                        gt_bboxes=None, expert_waypoints=None, stuck_detector=0, forced_move=False, fused_features=None):
        ...
        if fused_features is not None:
            ...
            print(fused_features.dtype)
            print("Tensor Device:")
            print(fused_features.device)
        ...
        # if fused_features is not None:
        # # Save fused features to a local path
        #     fused_features_path = os.path.join(save_path1, f'fused_features_step{step}.pt')
        #     torch.save(fused_features, fused_features_path)
        ...
        # Save fused features to a local path
        cv2.imwrite(str(save_path + ("/%d.png" % (step // 2))), images)
        # fused_features_path = os.path.join(save_path1, f'fused_features_step{step//2}.pt')
        # torch.save(fused_features, fused_features_path)
