...
import itertools
import pathlib
SAVE_PATH = "/home/ascc304/transfuser/logdir/out"
SAVE_PATH1 = "/home/ascc304/transfuser/logdir/pt"
...
if not SAVE_PATH1:
    ...
    pathlib.Path(SAVE_PATH1).mkdir(parents=True, exist_ok=True)
            ...
            for i in range(self.model_count):
                ...
                if (self.backbone == 'transFuser'):
                    pred_wp, _ = self.nets[i].forward_ego(image, lidar_bev, target_point, target_point_image, velocity,
                                                          num_points=num_points, save_path=SAVE_PATH, SAVE_PATH1=SAVE_PATH1, stuck_detector=self.stuck_detector,
                                                          forced_move=is_stuck, debug=self.config.debug, rgb_back=self.rgb_back)
                    # Save the fused_features tensor
                    #save_path = os.path.join(SAVE_PATH1, f'fused_features_step{self.step}.pt')
                    #torch.save(fused_features, save_path)
